...
(Прим.Пер - судя по всему, `totals` это сумма списаний/пополнений по всем транзакциям за определённую дату, а `counters` - количество этих транзакций).
...
Обратите внимание, как идеально вложены эти группы, и в любой момент времени мы смотрим только на один уровень или максимум на два соседних. Мы могли бы очень элегантно обработать такую логику, используя push-down стек или LIFO. Помните, это структура данных, которая растёт и убывает только с одного конца. Мы также должны решить, в какой структуре данных будут храниться все эти `counters` и `totals`. Одним из вариантов мог бы быть массив, но раз уж для каждого уровня мы итак должны в итоге вывести IP, содержащий значения для этого уровня, так почему бы не хранить значения прямо в IP все время? Этот тип IP называется _контрольным IP_, и его можно описать как **IP, время жизни которого точно соответствует времени жизни подпотока**, который он, можно сказать, представляет.

Теперь мы можем совместить эти техники и сделать вывод, что будем работать со стеком, содержащим контрольные IP (на самом деле их дескрипторы). Таким образом, приведенная выше логика становится намного проще. Теперь она читается примерно так:
